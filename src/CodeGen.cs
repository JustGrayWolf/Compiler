    ...
    public class CodeGen
    {
       ...
       // static int array_var_index = 0;
        ...
        public static void DropAll()
        {
            ...
            treeDepth = 0;
            allowRealToInt = false;
            exprResutType = "UNKNOWN";

        //static bool realFlag = false;
            real_constant_index = 0;
            string_constant_index = 0;
            label_index = 0;
        // static int array_var_index = 0;

            progName = "";
            verdec = new StringBuilder();
            mainCompStatement = new StringBuilder();
            varTypeList = new SortedList();
            typeDefinationList = new SortedList();
            labelDefList = new SortedList();
            constDefList = new SortedList();
        ...
        private static string generateAssignStatementCode(Node root)
        {
            ...
            if (destVarNode.Symbol.ToString().Equals("<array_select>"))//array variable
            {
                ...
                assStm.Append(generateCodeUpdateArrayName(tempDestVarName,destVarNode.getChild(1), false));
            ...
            if (tempDestVarType.Equals("SDWORD")) // destination variable or "the right side" is integer
            {
                ...
                else if ((expressionStr.Equals("<variable_name>")) || (expressionStr.Equals("<array_select>"))) // if the left side is a vairable
                {
                    ...
                    {
                        tempSrcVarName = strip(expression.getChild(0).getChild(0)).Replace("var$_$", "arr$_$");
                        assStm.Append(generateCodeUpdateArrayName(tempSrcVarName, expression.getChild(1)));
                        tempSrcVarName = generatedArrayName;
                        tempSrcVarType = generatedArrayType;
                    }
                    else
                    {
                    ...
                    {
                        assStm.Append("PUSH " + tempSrcVarName + "\n");
                        //assStm.Append("MOV EBX, " + tempSrcVarName + " \n");
                        //assStm.Append("MOV " + tempDestVarName + ", EBX \n");
                        assStm.Append("POP " + tempDestVarName + "\n");
                    }
        ...
        private static string generateCodeUpdateArrayName(string varName, Node IndexNode, bool is_src = true)
        {
            ...
            if (IndexNode.Symbol.ToString().Equals("<variable_name>")) //index is a variable
            {
                ...
                if (getVarType(indexVarName, IndexNode).Equals("SDWORD")) //index mustbe an integer
                {
                    ...
                    if (is_src)
                        storeIndexStr.Append("mov esi, eax\n"); // store the index to ebp as the function required
                    else
                        storeIndexStr.Append("mov edi, eax\n");
                    storeIndexStr.Append("pop ebx\n"); //restore eax, ebx
                    storeIndexStr.Append("pop eax\n");

                    if (is_src)
                        generatedArrayName = "dword ptr [esi]"; 
                    else
                        generatedArrayName = "dword ptr [edi]";

                }
                else
                    throw new CompilerException("Index of array must be an Integer", IndexNode);
            }
