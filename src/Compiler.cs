...
namespace Pascal
{
    public partial class Compiler : Form
    {
        ...
        private void compile()
        {
            ...
            try
            {
                cursorPosition.rowNumber = 1;
                cursorPosition.colNumber = 1;
                if (File.Exists("lastFile.pas"))
                {
                    // Удаляем файл
                    File.Delete("lastFile.pas");
                }
                richTextBoxInput.SaveFile("lastFile.pas", RichTextBoxStreamType.PlainText);
        ...
        private void buttonRun_Click(object sender, EventArgs e)
        {
            if (!f)
            {
                ...
                {
                    Console.WriteLine("Execute file not created");
                }
            }
            else
            {
                try
                {
                    ...
                    richTextBoxInput.SaveFile("lastFile.pas", RichTextBoxStreamType.PlainText);
                    string path = Application.StartupPath;
                    System.Diagnostics.Process.Start(path + "\\_run.bat");
                }
                catch
                {
                    Console.WriteLine("Error: File can't be save");
